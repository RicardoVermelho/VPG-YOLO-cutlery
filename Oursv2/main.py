...
def main(args):
    ...
    # Convert euler angles to axis angle and orientation matrix of a specified object
    def euler2axis(objectorientation):
        # --> Euler angles (alpha beta gamma) to axis angles
        alpha = objectorientation[0]
        beta = objectorientation[1]
        gamma = objectorientation[2]
    ...
    # Convert x,y,z to rotation matrix angles [roll, pitch, yaw] of a specified object
    def xyz2rotation(coords):
        rotx = math.atan2(coords[1],coords[2])
        if coords[2] >= 0:
            roty = -math.atan2(coords[0]*math.cos(rotx),coords[2])
        else:
            roty = math.atan2(coords[0]*math.cos(rotx),-coords[2])
    ...
    def process_actions():
        while True:
            ...
            if nonlocal_variables['executing_action']:
                ...
                # Get objects in simulation
                all_objects, orientation_allobjects = robot.get_obj_positions_and_orientations()
                copy_allobjects = all_objects.copy()
                # Filter for objects inside x-axis workspace
                ...
                '''Preprocess rgb image with yolo.
                Get either label of object to grasp or to push, and convert it to sim coords.
                Compare those coords to object positions in simulation and obtain closest object.'''
                bbox_path = home_path+'/inference/BBOX_xyxy/'
                action, objectlocation, objclass = preprocess(trainer.iteration, all_objects, bbox_path)
                idx_objectlocation = copy_allobjects.index(objectlocation)
                objectorientation = orientation_allobjects[idx_objectlocation]
                ...
                if action == 'grasp':
                    ...
                    #print('Action: %s at (%d, %d, %d)' % (action, objectlocation[0], objectlocation[1], objectlocation[2]))
                    ...
                    angle, obj_orientation_matrix = euler2axis(objectorientation)
                    rotationmatrix = xyz2rotation(objectlocation)
                    best_rotation_angle = rotationmatrix[2]

                    # Compute primitive position closer to the object2grab
                    #best_pix_ind, primitive_position = compute_position(heightmap_resolution, workspace_limits, new_valid_depth_heightmap, objectlocation, action, trainer, grasp_pred=grasp_predictions)
                    #nonlocal_variables['best_pix_ind'] = best_pix_ind

                    nonlocal_variables['grasp_success'] = robot.grasp(objectlocation, best_rotation_angle, workspace_limits, obj_class=objclass)
                    all_grasps += 1
                    ...
                    trainer.executed_action_log.append([1, objectlocation[0], objectlocation[1], objectlocation[2]]) # 1 - grasp
                    ...
                    #angle, obj_orientation_matrix = euler2axis(objectlocation)
                    #best_rotation_angle = np.deg2rad(nonlocal_variables['best_pix_ind'][0]*(360.0/trainer.model.num_rotations))
                    rotationmatrix = xyz2rotation(objectlocation)
                    best_rotation_angle = rotationmatrix[2]

                    previous_obj_pos = objectlocation

                    # Compute primitive position closer to the object2grab
                    #best_pix_ind, primitive_position = compute_position(heightmap_resolution, workspace_limits, new_valid_depth_heightmap, nonlocal_variables['primitive_action'], trainer, push_pred=push_predictions)
                    #nonlocal_variables['best_pix_ind'] = best_pix_ind

                    nonlocal_variables['push_success'] = robot.push(objectlocation, best_rotation_angle, workspace_limits)
                    all_pushes += 1
                    ...
                    trainer.executed_action_log.append([0, objectlocation[0], objectlocation[1], objectlocation[2]]) # 0 - push
                    logger.write_to_log('executed-action', trainer.executed_action_log)
