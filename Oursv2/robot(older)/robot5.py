...
class Robot(object):
    ...
    def add_objects(self):
        ...
        self.object_handles = []
        sim_obj_handles = []
        print("len obj mesh ind %f" % len(self.obj_mesh_ind))
        iteration_obj = 0
        for object_idx in range(len(self.obj_mesh_ind)):
            curr_mesh_file = os.path.join(self.obj_mesh_dir, self.mesh_list[self.obj_mesh_ind[object_idx]])
            print("curr_mesh_file: " + str(curr_mesh_file))

            if self.is_testing and self.test_preset_cases:
                print("\nI am testing...\n")
                curr_mesh_file = self.test_obj_mesh_files[object_idx]
                print("curr_mesh_file edited: " + str(curr_mesh_file))

            curr_shape_name = 'shape_%02d' % object_idx
            print("curr_shape_name: " + str(curr_shape_name))

            drop_x = (self.workspace_limits[0][1] - self.workspace_limits[0][0] - 0.2) * np.random.random_sample() + self.workspace_limits[0][0] + 0.1
            print("drop_x: " + str(drop_x))

            drop_y = (self.workspace_limits[1][1] - self.workspace_limits[1][0] - 0.2) * np.random.random_sample() + self.workspace_limits[1][0] + 0.1
            print("drop_y: " + str(drop_y))
            ...
            print("object_color: " + str(object_color))
            ...
            print("iteration_obj %f" % iteration_obj)
            iteration_obj = iteration_obj + 1
            print("-"*30)
    ...
    def restart_sim(self):
        ...
        print("gripper_position: " + str(gripper_position))
    ...
    def reposition_objects(self, workspace_limits):
        ...
        for object_handle in self.object_handles:
            ...
            object_position = [drop_x, drop_y, 0.15]
            object_orientation = [2*np.pi*np.random.random_sample(), 2*np.pi*np.random.random_sample(), 2*np.pi*np.random.random_sample()]
            vrep.simxSetObjectPosition(self.sim_client, object_handle, -1, object_position, v