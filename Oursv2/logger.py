...
class Logger():
    ...
    def __init__(self, continue_logging, logging_directory):
        ...
        if self.continue_logging:
            ...
            text_file = open(self.home_path+"/logs_latestFolder.txt", "w")
            text_file.write(timestamp_value.strftime('%Y-%m-%d.%H:%M:%S'))
            text_file.close()
            print('Creating data logging session: %s' % (self.base_directory))
        
        log_folder_file = open(self.home_path+"/logs_latestFolder.txt", "r")
        log_folder = str(log_folder_file.readline())
        ...
        self.visualizations_directory = os.path.join(self.home_path+'/logs/'+log_folder, 'visualizations')
    ...
    def save_visualizations(self, iteration, affordance_vis, name):
        log_folder_file = open(self.home_path+"/logs_latestFolder.txt", "r")
        log_folder = str(log_folder_file.readline())
        path_to_download_folder = str(os.path.join(Path.home(), "Downloads/VisualPushingGraspingV2/logs/"+log_folder+'/visualizations'))
        visualizations_path = path_to_download_folder
        cv2.imwrite(os.path.join(visualizations_path, '%06d.%s.png' % (iteration,name)), affordance_vis)
